          ...
          <div class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-6 bg-gray-50 p-4 rounded-lg">
              ...
              <div>
                ...
                <p class="font-medium">
                  {{ getEmployeeName(selectedOrder.EmployeeId) }}
                </p>
              </div>
              <div>
...
const fetchOrders = async () => {
  try {
    ...
    const customersResponse = await HttpService.getAxiosClient().get(
      "/Customer"
    );
    ...
    if (!customer) {
      console.error(
        `No customer found matching preferred_username: ${preferredUsername}`
      );
      return;
    }
